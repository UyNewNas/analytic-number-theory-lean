...
Project-neutral variation estimates for the complex weights `x ↦ x⁻ˢ` and
`x ↦ -log(x) x⁻ˢ`.  The proofs use the real-interval fundamental theorem of
calculus and norm bounds for interval integrals; no monotonicity assertion is
...
`MathlibNt/AnalyticNumberTheory/DirichletL/DirichletLAbelWeightVariation.lean`.
