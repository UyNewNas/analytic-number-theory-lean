...
Adapted, with provenance retained, from
`subfish-zhou/goldbach-lean@09b97db5764ade1246bfb77206baa1b124760958`,
`MathlibNt/AnalyticNumberTheory/DirichletL/DirichletLConditionalValueSeries.lean`.
The source and ANT use the same pinned mathlib revision
`e4c91783ca8e6a7c693ae624ade32fd22d4e43c1`.

The source's specialized finite Abel identity is not duplicated here: this
module reuses the project-neutral `AnalyticNumberTheory.Dirichlet.abel_Ico`
already extracted into `CharacterPrefix.lean`.
-/
...
lemma norm_sum_Ico_cpowWeight_character_le
    ...
    ‖∑ k ∈ Ico m n, cpowWeight s k * χ k‖ ≤
      q * (‖cpowWeight s (n - 1 : ℕ)‖ + ‖cpowWeight s m‖ +
        (‖s‖ / s.re) * (m : ℝ) ^ (-s.re)) := by
  have hab := abel_Ico χ (fun k : ℕ => cpowWeight s k) hmn
  rw [hab]
  calc
