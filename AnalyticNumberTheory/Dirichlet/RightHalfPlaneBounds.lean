...
/-!
# Elementary right-half-plane bounds for Dirichlet L-functions

This module extracts the minimal neutral absolute-convergence estimate needed by
the already-formalized finite-rectangle logarithmic-derivative argument.

Provenance: adapted from
`subfish-zhou/goldbach-lean@09b97db5764ade1246bfb77206baa1b124760958`,
`MathlibNt/AnalyticNumberTheory/DirichletL/DirichletLRightHalfPlaneBounds.lean`.
...
private theorem tsum_nat_add_one_rpow_neg_le (σ : ℝ) (hσ : 1 < σ) :
...
private theorem norm_dirichletLSeries_le {q : ℕ} (χ : DirichletCharacter ℂ q)
    (σ t : ℝ) (hσ : 1 < σ) :
...
/-- A Dirichlet L-function on the absolutely convergent half-plane is bounded
by the elementary zeta majorant, uniformly in modulus, character, and height. -/
