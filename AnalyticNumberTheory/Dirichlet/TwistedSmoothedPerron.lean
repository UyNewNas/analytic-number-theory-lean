...
/-!
# Twisted smoothed Perron inversion

This module is a provenance-preserving extraction of the project-neutral smoothed Perron
inversion formalized in
`subfish-zhou/goldbach-lean@09b97db5764ade1246bfb77206baa1b124760958`, module
`MathlibNt/AnalyticNumberTheory/DirichletL/DirichletLTwistedSmoothedPerron.lean`.

The analytic `-L'/L` seam is *not* duplicated here: it is reused from
`AnalyticNumberTheory.Dirichlet.VonMangoldtLSeries`.  The result is a neutral inversion API;
no GRH hypothesis, contour shift, Goldbach object, or downstream Liouville-reflection
...
/-- The coefficients of the negative logarithmic derivative of a Dirichlet L-function,
in the order used by the smoothed prime sum. -/
noncomputable def twistedVonMangoldtCoeff
    (χ : DirichletCharacter ℂ q) (n : ℕ) : ℂ :=
  (Λ n : ℂ) * χ n

/-- The genuinely infinite smoothed twisted Chebyshev sum. -/
noncomputable def twistedSmoothedPsi
    (χ : DirichletCharacter ℂ q) (ν : ℝ → ℝ) (ε X : ℝ) : ℂ :=
  ∑' n : ℕ, twistedVonMangoldtCoeff χ n * Smooth1 ν ε (n / X)

/-- The integrand in the twisted smoothed Perron formula. -/
noncomputable def twistedSmoothedPerronIntegrand
    (χ : DirichletCharacter ℂ q) (ν : ℝ → ℝ) (ε X : ℝ) (s : ℂ) : ℂ :=
  (-deriv (DirichletCharacter.LFunction χ) s /
      DirichletCharacter.LFunction χ s) *
    𝓜 (fun x ↦ (Smooth1 ν ε x : ℂ)) s * (X : ℂ) ^ s

private lemma negLogDerivLFunction_eq_tsum_twistedVonMangoldtCoeff
...
omit [NeZero q] in
private lemma twistedSmoothedPerron_aux_tsum_integral
  ...
  · rw [← lt_top_iff_ne_top]
    ...
    · rw [WithTop.lt_top_iff_ne_top, ENNReal.tsum_coe_ne_top_iff_summable_coe]
      ...
      refine Summable.of_nonneg_of_le
        (fun _ ↦ div_nonneg (norm_nonneg _) (by positivity))
        (fun n ↦ ?_)
...
set_option backward.isDefEq.respectTransparency false in
/-- Smoothed Perron inversion for von Mangoldt coefficients twisted by an arbitrary
Dirichlet character.  No nonprincipality assumption, GRH hypothesis, or contour shift is
used.  This is the neutral inversion theorem reused by downstream explicit-formula
arguments. -/
theorem twistedSmoothedPerron
  ...
  calc
    _ = 1 / (2 * π * I) * (I * ∫ t : ℝ, ∑' n : ℕ,
      ...
      congr
      ext t
      rw [negLogDerivLFunction_eq_tsum_twistedVonMangoldtCoeff χ]
