...
/-!
# Quantitative tails for the right Perron line

Provenance-preserving extraction from
`subfish-zhou/goldbach-lean@09b97db5764ade1246bfb77206baa1b124760958`, module
`MathlibNt/AnalyticNumberTheory/LargeSieve/DirichLTwistedSmoothedRightTailQuantitative.lean`.

The generic real-power tail estimate is reused from `RightHalfPlaneBounds`; it is not
re-proved here.  The theorem remains project-neutral: the constant depends only on the
fixed smoothing function and is chosen before the modulus, character, and contour
parameters.  No GRH or application-specific parameter choice appears.
-/
...
private lemma negLogDerivLFunction_eq_tsum_twistedVonMangoldtCoeff_tail
