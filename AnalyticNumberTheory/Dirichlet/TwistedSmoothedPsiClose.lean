...
/-!
# Character-neutral unsmoothing for the twisted von Mangoldt sum

This module is a provenance-preserving extraction of the transition-band estimate in
`subfish-zhou/goldbach-lean@09b97db5764ade1246bfb77206baa1b124760958`, module
`MathlibNt/AnalyticNumberTheory/LargeSieve/DirichLTwistedSmoothedPsiClose.lean`.

The external theorem is stated using Goldbach's `lambdaCharacterPrefix`.  ANT already has the
project-neutral cumulative sum `vonMangoldtCharacterPartialSum`, so we adapt only that finite
prefix representation and reuse the existing `PrimeNumberTheoremAnd.MediumPNT`
`SmoothedChebyshevClose_aux` machinery.  No Goldbach-specific sieve hierarchy, GRH assumption,
or downstream Liouville parameter is introduced here.
-/
...
omit [NeZero q] in
/-- The ANT cumulative twisted von-Mangoldt sum is the range sum of the coefficients used by
`twistedSmoothedPsi`.  The only additional point in `range (X+1)` is `0`, whose von-Mangoldt
coefficient vanishes. -/
theorem vonMangoldtCharacterPartialSum_eq_twistedCoeff_range
    (X : ℕ) (χ : DirichletCharacter ℂ q) :
    vonMangoldtCharacterPartialSum q X χ =
      ∑ n ∈ Finset.range (X + 1), twistedVonMangoldtCoeff χ n := by
  unfold vonMangoldtCharacterPartialSum twistedVonMangoldtCoeff
...
omit [NeZero q] in
/-- A termwise, twist-safe transition-band estimate.  This is the neutral core of Goldbach's
`DirichletCharacter.twistedSmoothedPsiClose_aux`, with the sharp prefix expressed directly by
ANT's `vonMangoldtCharacterPartialSum`. -/
theorem twistedSmoothedPsiClose_aux
  ...
  have htwistedFinite : twistedSmoothedPsi χ SmoothingF ε X =
    ...
    intro n hn
    simp [hFzero n hn]
  have hprefixFinite : vonMangoldtCharacterPartialSum q Y χ =
      ∑ n ∈ Finset.range N,
        twistedVonMangoldtCoeff χ n * (if n ≤ Y then (1 : ℝ) else 0) := by
    rw [vonMangoldtCharacterPartialSum_eq_twistedCoeff_range]
  ...
  have hpoint (n : ℕ) (hn : n ∈ Finset.range N) :
    ...
    have hnpos : 0 < n := Nat.pos_of_ne_zero hn0
    have hchar : ‖χ n‖ ≤ 1 := χ.norm_le_one n
...
omit [NeZero q] in
/-- The smoothed von-Mangoldt sum twisted by an arbitrary Dirichlet character is close to the
actual ANT cumulative character prefix through `⌊X⌋₊`.  The constant depends only on the fixed
smoothing function. -/
