...
| LS2 | Multiplicative large sieve: `Σ_{q≤Q} (q/φ(q)) Σ*_χ \|Σ_n a_n χ(n)\|² ≤ (Q²+N)Σ_n \|a_n\|²` | core lemmas landed, final assembly open (`LargeSieve/Multiplicative.lean`: 特征正交性 `charOrthSum`/`charOrthogonality_le` 用**全部特征**（mathlib `DirichletCharacter.sum_char_inv_mul_char_eq`，绕开 Gauss 和与原特征诱导）、有限傅里叶 Parseval `zmodParseval`（几何级数核 `geomSum_zmod_charReal`）、有理点集 `{r/q}` 的 `1/Q²`-分离 `rationalPoints_wellSpaced`——全部 kernel-checked；剩下把 Parseval 字符版装配进 `Σ_q Σ_{r<q} |S(r/q)|²` 并对该带重数点集做加法大筛（Montgomery 直接论证或重数吸收） | LS3 | 装配 Parseval 的字符版本（`Σ_{x mod q}|Σ_{n≡x} a_n|² ≤ (1/q)Σ_{r<q}|S(r/q)|²`），再把 `Σ_{q≤Q} Σ_{r<q} |S(r/q)|²` 用加法大筛控制（含重数处理） |
